...
//   // 修改服务客户端名称，指向红色检测服务
//   ros::ServiceClient red_client = nh.serviceClient<std_srvs::Trigger>(
//       "/analyse_red"); // 红色检测服务客户端
//   ros::Publisher qr_pub =
//       nh.advertise<std_msgs::String>("/red_detection_content", 10); // 发布红色检测信息
...
//       if (is_hanging)
//       {
//         // 在悬停开始时调用红色检测服务（只调用一次）
//         if (!qr_scan_called)
//         {
//           ROS_INFO("Calling red detection service...");
//           std_srvs::Trigger srv;

//           if (red_client.call(srv))
//           {
//             if (srv.response.success)
//             {
//               ROS_INFO("Red detection successful: %s",
...
//               ROS_INFO("Published red detection content: %s", qr_msg.data.c_str());
//             }
//             else
//             {
//               ROS_WARN("Red detection failed: %s",
//                        srv.response.message.c_str());

//               // 即使失败也发布空消息或错误信息
//               std_msgs::String qr_msg;
//               qr_msg.data = "RED_DETECTION_FAILED: " + srv.response.message;
//               qr_pub.publish(qr_msg);
//             }
//           }
//           else
//           {
//             ROS_ERROR("Failed to call red detection service");

//             // 服务调用失败时发布错误信息
//             std_msgs::String qr_msg;
//             qr_msg.data = "RED_SERVICE_UNAVAILABLE";
//             qr_pub.publish(qr_msg);
//           }
...
int main(int argc, char **argv)
{
  ...
  // 获取当前焦距
  double focus = cap.get(cv::CAP_PROP_FOCUS);
  ROS_INFO("当前焦距: %f", focus);
  if (focus > 0) {
    ROS_INFO("当前焦距: %f", focus);
  } else {
    ROS_ERROR("无法获取摄像头焦距");
  ...
  // 修改服务客户端名称，指向红色检测服务
  ros::ServiceClient red_client = nh.serviceClient<std_srvs::Trigger>(
      "/analyse_black"); // 红色检测服务客户端
  ros::Publisher qr_pub =
      nh.advertise<std_msgs::String>("/black_detection_content", 10); // 发布红色检测信息

  // 等待红色检测服务启动
  ros::service::waitForService("/analyse_black");
  ...
  if (red_client.call(srv))
  {
    if (srv.response.success)
    {
      ROS_INFO("black detection successful: %s",
               srv.response.message.c_str());
      ...
      qr_msg.data = srv.response.message;
      qr_pub.publish(qr_msg);
      ROS_INFO("Published black detection content: %s", qr_msg.data.c_str());
    }
    else
    {
      ROS_WARN("black detection failed: %s",
               srv.response.message.c_str());

      // 即使失败也发布空消息或错误信息
      std_msgs::String qr_msg;
      qr_msg.data = "black_DETECTION_FAILED: " + srv.response.message;
      qr_pub.publish(qr_msg);
    }
  }
  else
  {
    ROS_ERROR("Failed to call black detection service");

    // 服务调用失败时发布错误信息
    std_msgs::String qr_msg;
    qr_msg.data = "black_SERVICE_UNAVAILABLE";
    qr_pub.publish(qr_msg);
  }
